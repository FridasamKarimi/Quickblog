...
const AddBlog = () => {
  ...
  const generateContent = async () => {
    if(!title) return toast.error('Please enter a title')
      ...
      try {
        setLoading(true);
        const { data } = await axios.post('/api/blog/genarate', {prompt: title})
        if (data.success){
  ...
  const onSubmitHandler = async (e) => {
    try {
      ...
      const blog = {
        title, subTitle,
        descrription: quillRef.current.root.innerHTML,
  ...
  return (
        <form onSubmit={onSubmitHandler} className='flex-1 bg-blue-50/50 text-gray-600 overflow-scroll'>
          ...
          <div className='bg-white w-full max-w-3xl p-4 md:p-10 sm:m-10 shadow rounded'>
            ...
            <label htmlFor='image'>
              <img src={!image ? assets.upload_area : URL.createObjectURL(image)} alt="" className='m2 h-16 rounded cursor-pointer'/>
              <input onChange={(e) => setImage(e.target.files[0])} type="file" id='image' className='hidden' name='image' hidden required />
            </label>
            ...
            <div className='max-w-lg h-74 pb-16 sm:pb-10 pt-2 relative'>
              ...
              {loading && (
                <div className='absolute right-0 top-0 bottom-0 lefft-0 flex
